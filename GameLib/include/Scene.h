...
public:
   ...
   void UpdateScene(float deltaTime) {
       Update();
        ...
        dispatchCollisions();
        // Pass 3: Late update / rendering (all objects see the final camera
   ...
   /// Push a scene on top (this scene stays alive underneath).
   void PushScene(Scene* scene) {
       m_engine->PushScene(scene);
   }

   /// Pop this scene and return to the previous one.
   void PopScene() {
       m_engine->PopScene();
   }
